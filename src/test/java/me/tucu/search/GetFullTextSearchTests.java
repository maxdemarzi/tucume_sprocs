...
public class GetFullTextSearchTests {
...
//            Result schema = session.run("CALL me.tucu.schema.create();");
  //          assertThat(schema.list().size(), equalTo(5));

            // When I use the procedure
            Result result = session.run( "CALL me.tucu.search.fulltext.get($term);",
    ...
    private static final String INDEX = "CALL db.index.fulltext.createNodeIndex('fulltext', ['Post','User','Product','Tag'], ['status','username','name'])";
